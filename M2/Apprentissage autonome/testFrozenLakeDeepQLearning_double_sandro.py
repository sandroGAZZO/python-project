...
class NN:
    def __init__(self,sizeInput,sizeHiddenLayer,sizeHiddenLayer2,sizeOutput):
        ...
        self.HiddenLayerEntryWeights2=numpy.zeros([sizeHiddenLayer2,sizeInput])
        self.LastLayerEntryWeights2=numpy.zeros([sizeOutput,sizeHiddenLayer2])
        ...
        for i in range(0,sizeHiddenLayer2):
            for j in range(0,sizeInput):
                self.HiddenLayerEntryWeights2[i,j]=random.uniform(-0.1,0.1)
        ...
        for i in range(0,sizeOutput):
            ...
            for j in range(0,sizeHiddenLayer2):
                self.LastLayerEntryWeights2[i,j]=random.uniform(-0.1,0.1)
        ...
        self.HiddenLayerEntryDeltas2=numpy.zeros(sizeHiddenLayer2)
        self.LastLayerEntryDeltas2=numpy.zeros(sizeOutput)
        
        self.HiddenLayerOutput2=numpy.zeros(sizeHiddenLayer2)
        self.LastLayerOutput2=numpy.zeros(sizeOutput)
    ...
    def output(self,x):
        ...
        for i in range(0, self.sizeHiddenLayer2):
            self.HiddenLayerOutput2[i]=sigmoid(numpy.dot(self.HiddenLayerEntryWeights2[i],self.HiddenLayerOutput))
            
        for i in range(0, self.sizeOutput):
            self.LastLayerOutput[i]= \
            sigmoid(numpy.dot(self.LastLayerEntryWeights[i],self.HiddenLayerOutput2))
    ...
    def retropropagation(self,x,y,actionIndex):
        ...
        self.LastLayerEntryDeltas2[actionIndex]=2*(self.LastLayerOutput2[actionIndex]-y)* \
            (1+self.LastLayerOutput2[actionIndex])*(1-self.LastLayerOutput2[actionIndex])
            
        for i in range(0,self.sizeHiddenLayer2):
            #here usually you need a sum
            self.HiddenLayerEntryDeltas2[i]=self.LastLayerEntryDeltas2[actionIndex]* \
            (1+self.HiddenLayerOutput2[i])*(1-self.HiddenLayerOutput2[i])*self.LastLayerEntryWeights2[actionIndex,i]
            
        for i in range(0,self.sizeHiddenLayer2):
            self.LastLayerEntryWeights2[actionIndex,i]-=alpha*self.LastLayerEntryDeltas2[actionIndex]* \
            self.HiddenLayerOutput2[i]
            
        for i in range(0,self.sizeHiddenLayer2):
            for j in range(0,self.sizeInput):
                self.HiddenLayerEntryWeights2[i,j]-=alpha*self.HiddenLayerEntryDeltas2[i]*x[j]
...
sizeHiddenLayer2=10
sizeOutput=4
myNN = NN(sizeInput, sizeHiddenLayer,sizeHiddenLayer2, sizeOutput)
